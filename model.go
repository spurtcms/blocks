...
func (Blockmodel BlockModel) CollectionList(offset int, limit int, filter Filter, DB *gorm.DB, tenantid int) (collection []TblBlock, totalcollection int64, err error) {
	...
	query := DB.Table("tbl_blocks").Select("tbl_blocks.*").Joins("inner join tbl_block_collections on tbl_block_collections.block_id = tbl_block.id").Where("tbl_block_collection.tenant_id=? and tbl_block_collections.is_deleted = ? and tbl_block_collection.user_id ", tenantid, 0, Blockmodel.UserId).Group("tbl_ecom_customers.id, tbl_ecom_product_orders.customer_id").Order("tbl_block.id desc")
